...
\item Let $S = \{v_1, v_2, \dots, v_k\}$ be a subset of $k$ 
  linearly independent vectors.  How many
  such subsets are there in $V$? (Hint: using (b), count the
  number of ways to build such sets.)
...
\begin{problem}
  ...
  \begin{enumerate}[(a)]
  \item Let $A \in \mathcal{M}_{n\times n}(F)$, let $W \leq F^n$ be a
    ...
    (Hint: consider the minimal polynomials of $A$ and $A{\mid_W}$.)
...
\begin{problem}
  ...
  \item For this part, assume that $V$ is finitely generated over $F$. 
    ...
    then $U = \bigcap\limits_{i=1}^r \ker{\theta_i}$. (Hint: decompose 
    $V = U \oplus U'$.)
