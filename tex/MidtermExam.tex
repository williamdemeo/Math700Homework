...
giving complete justifications for all arguments.
Since this is a take-home exam, I expect the presentation quality to be higher
than if it were an in-class exam.
...
\noindent {\bf HONOR CODE:} You are expected to solve the exam problems on your own
  ...
  If you get stuck, please ask \emph{me} for help.  I am happy to discuss the
  problems with you, and I may post hints on our wiki page. 
...
$\Hom(V,W)$ into $\Aff(V,W)^W$. Is $\zeta$ a vector space
...
\item The sets
  ...
  \item If $T_i$ is the restriction of $T$ to $V_i$, then 
    $T_1$ is an isomorphism and $T_2$ is \emph{nilpotent}.\footnote{An
    endomorphism $\alpha\in \End(V)$ is called \emph{nilpotent} if there is a
    positive integer $k$ such that $\alpha^k = \sigma_0$.}
  \end{enumerate}
\end{problem}
